...
`epoll_wait(epfd, events, MAX_EVENTS, -1)` returns -1. errno: EINTR. The system call did not fail. The system call was interrupted by signal delivery.
...
The listening socket, fd 3, remains bound to 0.0.0.0:8402. The new port value is 9100. The process does not call `bind()` again. The socket is already in LISTEN state. The new port value occupies the configuration struct. The socket occupies the kernel's socket table. These are different data structures.

`close(7)`. The configuration file descriptor is released.

---

Log rotation. The process calls `close(4)`. File descriptor 4 — the log file, opened during initialization — is released. The kernel decrements the reference count on the underlying inode. If an external process has renamed `/var/log/daemon.log` to `/var/log/daemon.log.1`, the close releases the last reference to that inode.
...
PID 48891. The same PID. The same session ID. The same listening socket on fd 3, bound to 0.0.0.0:8402. The configuration struct holds port 9100, but the socket holds 8402. The log file descriptor is 4 — the same integer, potentially a different inode. The process executes the same code path. The data in memory has changed. The code has not.
